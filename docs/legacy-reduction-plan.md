...
**v16 update.** Item 5's *"final Phase-1 expansion"* label is layered-historical — empirically, an additional Phase-1-shape expansion shipped after the original Phase 1 framing was drafted. Item 5b below records the addition (sub-numbered to avoid colliding with Phase 2's existing item 6 numbering); original item 5 prose preserved per the SPEC §13.6/§13.8 layering discipline. Drift caught at v16 composition-time drift-sweep per SPEC §14.3.

5b. ✅ **`assumptions` section** (added for [#24](https://github.com/isaint-jean/cre-credit-committee/issues/24), no specific D-tag) — **SHIPPED at render version 7.3.** Top-level named-field struct (`RenderedAssumptionsSection`) mirroring `AdjustedInputs.assumptions`: 4 fields (`capRate`, `terminalCapRate`, `rentGrowthPct`, `expenseGrowthPct`), each a `RenderedLineItem`. Bijective passthrough via the existing `projectLineItem` helper; render-side discipline preserved (no recomputation). The Phase-1-shape work that item 5's framing inadvertently labeled "final" — captures the additive parity expansion that v12 #24 shipped (per #24; see SPEC §10.14). **Editable affordance**: also extends frontend `EditableLineItemTable` for the new assumptions section, opening the Phase 4 mechanism additively in the same commit — first amendment to use the Phase 4 mechanisms for fields WITH engine builders (capRate via `buildCapRate`, terminalCapRate via `buildTerminalCapRate`, rentGrowthPct, expenseGrowthPct). SPEC §14.3 / v16 will extend the same mechanism additively for a field WITHOUT an engine builder (per Delta S handbook constraint).
...
**v16 update.** Phase 4's *"DEFERRED — do not begin until... the architectural domain of writable rendered semantics has been explicitly opened"* framing is layered-historical. Empirically, the constituent mechanisms have shipped piece by piece since #20 (revision-lineage system / `applyRevisionDelta`) and v12 #24 (`EditableLineItemTable` + `EDITABLE_PATHS` whitelist for the assumptions section + frontend edit affordance per #24 ship; see item 5b above). The "architectural domain" opening wasn't formally invoked as a single moment; it happened additively through incremental shipping. The constituent mechanisms shipped to date: revision-delta with editable whitelist (#20 / commit 8.5); `AdjustedLineItem` dual-purpose pattern (engine-computed AND analyst-editable simultaneously); `EditableLineItemTable` frontend affordance; `EDITABLE_PATHS` + `PERCENT_PATHS` + `YEARS_PATHS` whitelists (backend `apply-revision-delta.ts` + frontend `uw-edit-utils.ts`). SPEC §14.3 / v16 is the first amendment to use the Phase 4 mechanisms for a field WITHOUT an engine builder (I9 concluded cap rate per Delta S handbook constraint — analyst-input-only, no `buildConcludedCapRate` analog to `buildCapRate`); v12 #24 was the first to use the same mechanisms for fields WITH engine builders. The cumulative shape of these expansions is the de-facto Phase 4 opening; the original "explicit decision required" directive above is preserved per the SPEC §13.6/§13.8 layering discipline as the historical-record of the original framing.
