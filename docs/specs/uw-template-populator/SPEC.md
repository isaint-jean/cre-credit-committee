...
- **v15 — 2026-06-08.** AppraisalExtraction producer scoping session. §14.2 decisions document drafted (replacing the v8 §14.2 placeholder), capturing 7 contract-design decisions across architectural-shape (3), phase-scope (3), and one detailed-deferred (anchor fixture); 3 additional deferred decisions listed without per-decision detail (C14 disposition, joint-extractor vs separate-call architecture, adapter pattern fit) — all four deferrals explicit for next-session pickup. **Critical scoping reframing — Delta P**: the current AppraisalExtraction contract (3 fields: valueConclusion, capRate, methodology) serves the *valuation anchor* purpose only — its outputs flow through narrative-facts → valuation engine for the final-value anchor. The Bucket 4 operating-statement cells (13) and Bucket 6 descriptor cells (8) that §5.2 attributed to the "appraisal producer workstream" are NOT addressable from the 3-field contract; they require a multi-record producer that writes to AppraisalExtraction (anchor-fields widening) + a new operating-statement sub-record (Phase 2) + PropertyMetadata extensions (Phase 3). The "what TWO things might this contract be doing?" recon discipline from Delta K applied at the contract-design layer surfaced the reframing. The original naive scoping mental-model — "mirror PCA: widen AppraisalExtraction from 3 to ~12 fields, single ticket" — would have mis-framed the work by a wide margin; the actual scope spans 3 sub-records across 3 implementation tickets. §5.2 receives a v15 layered note acknowledging the multi-record reframing without rewriting the original ghost-contract framing per the §13.6/§13.8 layering discipline. §11.4 receives a new item 6 (appended to avoid renumbering items 4 + 5) covering the appraisal producer sequencing — multi-record scope, 3-phase split, I9 separation, anchor fixture deferred, comparable cell-unlock count to PCA's full arc (~21 cells across Bucket 4 + Bucket 6 + valuation anchor extensions). **Delta Q** (Step 2 of v15 orient): §11.4 didn't pre-stage appraisal-producer sequencing — PCA got item 1, OperatingStatementExtraction got item 2, I9 got item 3, but appraisal was mentioned only in passing in item 1's C14 carve-out. v15 closes that documentation gap. No §10 entry per the v11 precedent for docs-only design/scoping amendments — substantive narrative carried by this revision history entry + §14.2 itself; §10 is reserved for behavior-change documentation, and v15 ships zero production code. Two new deltas captured inline in §14.2's opening paragraph rather than as new §13.x entries: Delta P (contract-design-layer manifestation of the §13.5 "what TWO things" pattern; the AppraisalExtraction contract was hiding the multi-record scope under its single-contract name) and Delta Q (§11.4 sequencing documentation gap — same §13.8 spec-stub-currency family; the original §11.4 drafting predated the producer-by-producer scoping cadence the project adopted). `JUDGMENT_ENGINE_VERSION` stays at `'1.2'`; no EEV bump; no contract changes; v15 is a pure design/scoping amendment per the original v8 PCA-scoping precedent (which was also docs-only). v15 amendment commit therefore lands as docs-only; no SHIP-HASH or other code-side placeholders to resolve.
- **v16 — 2026-06-09 (this revision).** I9 Concluded Cap Rate scoping session. §14.3 decisions document drafted (third contract-decisions section after §14.1 PCA and §14.2 Appraisal), capturing 6 decisions taken + 3 deferred. **Critical scoping reframing — Delta S**: I9 is NOT producer-shape per the P-III-9 handbook principle's explicit *"No deterministic threshold — handbook does not give one, and inventing one is out of scope"* directive. §11.4 item 3's prior "Greenfield: new contract slot + new builder + new doctrine wiring" framing assumed producer scope; recon surfaced the handbook constraint and the "what TWO things might X be doing?" pattern applied at the cell-design layer surfaced the reframing. §11.4 item 3 receives a v16 layered note (Delta T closure) acknowledging the reclassification without rewriting per §13.6/§13.8 layering discipline. I9 ships as a single-ticket extension of the v12 #24 pattern — new `concludedCapRate: AdjustedLineItem` field on `AdjustedAssumptions`, analyst-editable via the existing revision-delta + EDITABLE_PATHS + EditableLineItemTable infrastructure shipped post-#20 / v12 #24. RENDER_VERSION will bump `'7.3'` → `'7.4'` when the implementation ticket ships; v16 itself is pure design/scoping per the v8 PCA-scoping + v15 Appraisal-scoping precedent (docs-only). **Drift-sweep discipline empirically validated.** Yesterday's v15 closure surfaced Delta U as a narrow finding (legacy-reduction-plan.md §7 Phase 4 framing stale). Today's v16 composition-time drift-sweep (operationalizing yesterday's closing meta-observation) surfaced a BROADER Delta U scope — `legacy-reduction-plan.md` Phase 1 list ends at item 5 (D04 at RENDER_VERSION 7.2) with the label "final Phase-1 expansion," but v12 #24 shipped an additional Phase-1-shape expansion at 7.3 (AdjustedAssumptions render-side projection) that the doc's list never captured. Two-touch refresh in `legacy-reduction-plan.md` (Delta U closure — Phase 1 item 6 addition for #24/7.3 + Phase 4 layered note acknowledging constituent mechanisms have opened piece-by-piece since #20/#24); both touches preserve original framings per the layering discipline established for SPEC §11.4 / §13.6 / §5.2 / §13.8 layered notes. **Drift-sweep as forward-discipline candidate**: yesterday's closing meta-observation proposed *"before composing a new amendment, sweep related doc surfaces for drift"*. Today's sweep validated the discipline — a 5-minute Step 1 read across 6 doc surfaces yielded one new finding (broader Delta U) that yesterday's narrow framing missed. Empirical evidence the discipline catches drift faster than per-session natural reads. The discipline doesn't get a new §13.x entry today (consistent with v15's Delta-P/Q-as-§13.5/§13.8-family treatment); the §14.3 opening paragraph records it as substantive contribution. §14.3 uses **flat 6-decision structure** (returning to §14.1's pattern) since I9 is single-phase per Delta S; the structural variation is honest about decision-kind variation across §14.x entries (§14.1 flat / §14.2 3-category mixed-kinds / §14.3 flat single-phase) — future §14.x entries pick structure to match actual decision-kind variation. Four deltas now reference the established R/S/T/U set: R (legacy populator name collision), S (P-III-9 LLM_CONTEXT reclassification), T (§11.4 item 3 framing stale per Delta S), U (legacy-reduction-plan.md two-doc-surface drift — Phase 1 and Phase 4). No §10 entry per the v11 precedent for docs-only design/scoping amendments — substantive narrative carried by this revision history entry + §14.3 itself + the §11.4 item 3 layered note + the legacy-reduction-plan.md two-touch refresh; v16 ships zero production code. `JUDGMENT_ENGINE_VERSION` stays at `'1.2'`; no EEV bump; no contract changes; v16 is the third docs-only design/scoping amendment in the project's lifecycle (v8 PCA scoping, v15 Appraisal scoping, v16 I9 scoping). v16 amendment commit therefore lands as docs-only across two files (SPEC.md + legacy-reduction-plan.md per the v11 light-integration discipline keeping SPEC and operational doc synchronized); no SHIP-HASH or other code-side placeholders to resolve.
...
3. **I9 Concluded Cap Rate as a Tier B ticket.** Highest single-cell consequence in the template per registry notes. Greenfield: new contract slot (e.g., `concludedCapRate` field), new builder, new doctrine wiring. Significantly larger scope than D.3 — closer to "a mini-feature" than "a wire-up ticket." Has Tier-B-on-Tier-B dependencies (§11.3) that should be settled before scoping.
   ...
   **Update (v16).** I9 reclassified out of producer-shape per Delta S (§14.3) — the v6 framing of "Greenfield: new contract slot + new builder + new doctrine wiring" assumed producer/builder scope. P-III-9 handbook principle explicitly says *"No deterministic threshold — handbook does not give one, and inventing one is out of scope"* (`packages/handbook-data/src/handbook.ts:556` notes). I9 ships as a single-ticket extension of the v12 #24 pattern (new `concludedCapRate: AdjustedLineItem` field on `AdjustedAssumptions`, analyst-editable via existing revision-delta + EDITABLE_PATHS + EditableLineItemTable infrastructure). See §14.3 for the 6 decisions taken + 3 deferred and the R/S/T/U delta capture. Original v6 framing above preserved per the §13.6/§13.8 layering discipline; v16 reclassifies scope without rewriting history.
...
### 14.3 I9 Concluded Cap Rate — single-ticket extension of the v12 #24 editable-rendered pattern

Scoped during the v16 session against the I9 cell (`Conclusions & Escrows` tab, named range `Concluded_Cap_Rate`, registry tier B, sunroad_example 0.085). Recon-then-design pattern matching §14.1 (v8 PCA) and §14.2 (v15 Appraisal): the morning's recon surfaced the empirical findings (current `concludedCapRate` mapping name exists in the LEGACY populator with wrong-value semantics; P-III-9 handbook principle explicitly says no deterministic threshold; §11.4 item 3's "Greenfield + new builder + new doctrine" framing is stale relative to the handbook constraint); the afternoon's contract-design conversation resolved six design choices against the reframed scope this section records. **Flat 6-decision structure** (returning to §14.1's pattern, departing from §14.2's 3-category mixed-kinds structure) because I9 is single-phase per Delta S — no phase-scope subsection earns its own grouping when there's only one phase. Structural variation across §14.x entries is honest about decision-kind variation (§14.1 flat / §14.2 3-category / §14.3 flat single-phase); future §14.x entries pick structure to match actual decision-kind variation.

**Critical opening note — Delta S contract-design + cell-design reframing.** §11.4 item 3 (carried since v6) framed I9 as: *"Greenfield: new contract slot (e.g., `concludedCapRate` field), new builder, new doctrine wiring. Significantly larger scope than D.3 — closer to 'a mini-feature' than 'a wire-up ticket.'"* The v16 recon (Step 1) surfaced that this framing assumes producer/builder shape. The P-III-9 handbook principle at `packages/handbook-data/src/handbook.ts:556` is explicit on the constraint: principle text *"Assess value using both stabilized and stressed cap rate assumptions"*; `executionModes: ['LLM_CONTEXT']`; notes — *"No deterministic threshold — handbook does not give one, and inventing one is out of scope."* I9 is NOT computationally derivable; it's an underwriter-input cell with handbook-context display (the "TWO things" the §11.4 framing was hiding under one cell-design layer: underwriter-input AND handbook-context-injection, not producer-output). The "what TWO things might X be doing?" recon discipline from §10.16 Delta K (v14) → §14.2 Delta P (v15) → §14.3 Delta S (v16) now spans three layers: recon-implementation / contract-design / cell-design. Each scoping-then-implement arc has extended the pattern to one more layer of abstraction.

**Drift-sweep discipline empirical validation (v16 Step 1).** Yesterday's v15 closure surfaced Delta U as a narrow finding (legacy-reduction-plan.md §7 Phase 4 "DEFERRED" framing stale) and proposed *"before composing a new amendment, sweep related doc surfaces for drift"* as forward-discipline candidate. Today's v16 Step 1 drift-sweep operationalized the candidate — a focused 5-minute read across 6 doc surfaces (SPEC §11.4 / §14.1 / §14.2 / legacy-reduction-plan.md §7 / SPEC cross-references / SPEC revision history) yielded one NEW finding the yesterday's narrow framing missed: `legacy-reduction-plan.md` Phase 1 list ends at item 5 (D04 at RENDER_VERSION 7.2) with the label *"final Phase-1 expansion"*, but v12 #24 (AdjustedAssumptions render-side projection at 7.3) was an additional Phase-1-shape expansion the doc's list never captured. The Phase 1 drift compounds the Phase 4 drift in the same doc — Delta U is two-touch, not one-touch. The drift-sweep caught the broader scope at composition-time-before-prose, where per-session natural reads during the v15 close-out hadn't surfaced it. **First documented empirical-confirmation of drift-sweep discipline value.** The discipline doesn't earn a new §13.x entry (consistent with v15's Delta-P/Q-as-§13.5/§13.8-family treatment); this paragraph IS the substantive contribution recording it. Forward-discipline crystallized: amendment composition's Step 1 should include explicit drift-sweep across related-doc-surfaces, not just the doc being amended.

**Decision 1 — I9 framing reclassification**

CHOSEN: accept Delta S; I9 is NOT producer-shape; reclassify out of §11.4 item 3's "greenfield + new builder + new doctrine" framing.

Schema: no new field shape decision under this item; the reclassification is the foundation for Decisions 2-6.

Rationale: P-III-9 handbook principle explicitly disallows deterministic threshold derivation — *"No deterministic threshold — handbook does not give one, and inventing one is out of scope."* Designing a builder for a non-computable value would mis-frame the work and produce square-peg-round-hole code paths. The reframing aligns I9 with the editable-rendered-cell family rather than the producer family. §11.4 item 3 receives a v16 layered note (Delta T closure) acknowledging the reclassification without rewriting; original v6 framing preserved per §13.6/§13.8 layering discipline.

**Decision 2 — Producer surface**

CHOSEN: (γ) target — cell + handbook-context panel. (α) cell-only is acceptable fallback if handbook-context infrastructure is heavy lift at implementation time.

Schema: the cell itself is an editable surface; the handbook-context panel renders P-III-9's stabilized + stressed cap rate considerations alongside.

Rationale: P-III-9's principle text — *"Assess value using both stabilized and stressed cap rate assumptions"* — paired with its LLM_CONTEXT execution mode, means the underwriter needs visibility into the dual-consideration framing AT the cell-input surface. Cell-in-isolation (option (α)) loses that context-proximity. (γ) is the right architectural target; (α) is fallback because handbook-context infrastructure may not exist as primitive today.

**Decision 3 — Contract slot location**

CHOSEN: (a) RESCOPED — new field `concludedCapRate: AdjustedLineItem` on `AdjustedAssumptions`.

Schema:
```ts
// In packages/contracts/src/adjusted-inputs.ts, AdjustedAssumptions:
export interface AdjustedAssumptions {
  readonly capRate: AdjustedLineItem;            // going-in: engine builder + analyst-editable
  readonly terminalCapRate: AdjustedLineItem;    // exit: engine builder + analyst-editable
  readonly concludedCapRate: AdjustedLineItem;   // analyst-judgment ONLY (no engine builder per Delta S)
  readonly rentGrowthPct: AdjustedLineItem;
  readonly expenseGrowthPct: AdjustedLineItem;
}
```

Default state per Delta S (handbook says no deterministic threshold):
```ts
// concludedCapRate default when unset by analyst:
{ raw: null, adjusted: null, source: 'ANALYST_JUDGMENT' /* new SourceTier? */, adjustments: [] }
```

Rationale: my initial framing of (a) as *"semantically wrong (AdjustedAssumptions is engine-computed, not underwriter-input)"* was empirically wrong — verification (v16 Step 1) surfaced that AdjustedAssumptions fields are simultaneously engine-computed AND analyst-editable via the AdjustedLineItem dual-purpose pattern. `assumptions.capRate.adjusted` is already in `EDITABLE_PATHS` (both backend `apply-revision-delta.ts` and frontend `uw-edit-utils.ts`); the going-in cap rate has an engine builder (`buildCapRate`) AND analyst-editable override. I9's `concludedCapRate` fits naturally as a sibling field with the caveat that it has NO engine builder (per Delta S) — `raw` and `adjusted` are `null` by default; only the editable surface populates them. (b) new ConcludedJudgments record was rejected because it would parallel-track AdjustedInputs without integrating with the revision-lineage system. (c) EditableOverlay path was rejected because overlays are metadata-not-replacement per the schema's explicit comment (*"The original render output remains unchanged - the override is metadata, not a replacement"*); I9's concluded cap rate IS the value downstream consumers use, not advisory commentary.

Implementation hooks: add `assumptions.concludedCapRate.adjusted` to backend `EDITABLE_PATHS` in `apply-revision-delta.ts` + frontend `EDITABLE_PATHS` + `PERCENT_PATHS` (cap rates are 0..1 fractions) in `apps/web/src/lib/uw-edit-utils.ts`. Render-side `RenderedAssumptionsSection` extends 4 → 5 fields. RENDER_VERSION bumps `'7.3'` → `'7.4'` (id-space rotation — new section shape; cache wrapper auto-handles the version split per the established pattern).

**Decision 4 — NOI sequencing**

CHOSEN: explicit underwriter-choice at edit time.

Schema: no new contract field; the existing Conclusions & Escrows tab carries the spreadsheet-formula `= NOI × I9 → Concluded Value`. The render surface shows NOI candidates as context (seller UW NOI, bank's stressed UW NOI, etc. — already projected via `RenderedAnalysis` per Phase 1 shipped expansions); the analyst's I9 input implicitly resolves which NOI it applies to via spreadsheet-side multiplication.

Rationale: §11.3 Tier-B-on-Tier-B dependency framing (*"set I9 against the seller's UW NOI vs. set I9 against the bank's stressed UW NOI is a sequencing decision"*) is genuine but doesn't need to be pre-resolved at the contract layer — it's an UI-time underwriter judgment. The contract carries the analyst's chosen cap rate; the spreadsheet does the multiplication; the underwriter's interpretation of "which NOI" is implicit in their cap rate choice. Doesn't pre-commit the system to one NOI-interpretation; doesn't entangle I9 scoping with Tier-B-on-Tier-B execution-ordering design (which remains §11.3's deferred-architectural territory).

**Decision 5 — Legacy populator collision handling**

CHOSEN: (i) leave legacy `template-engine.service.ts:756` untouched; new-spine I9 ships in parallel; legacy retires per the §12 sunset path.

Schema: no legacy code changes; the `concludedCapRate` mapping name in the legacy populator remains as a documented compromise (legacy populator writes `uwModel.capRate` going-in proxy → I9 cell; legacy retires as a unit per §12 / `legacy-reduction-plan.md`).

Rationale: per-cell legacy patches add fragility and create dual-source-of-truth across legacy vs new-spine. The §12 graduation framework (v11) committed to legacy retirement as a unit, not per-cell. Coexistence during the sunset window is the established pattern. Delta R (legacy name collision) is therefore handled by NOT touching the legacy code path; the name collision is structural (same string `concludedCapRate` in two places with different semantics) but operationally harmless during the sunset window since the two surfaces don't share state.

**Decision 6 — Concluded Value disposition**

CHOSEN: stay with spreadsheet formula; out-of-band for this scoping session.

Schema: no contract field for "Concluded Value"; the cell `Concluded_Value` in the Conclusions & Escrows tab is computed via spreadsheet formula `= NOI × I9`. The producer/contract layer doesn't ship Concluded Value as a derived field.

Rationale: Concluded Value is a presentation-layer derivation; pulling it into the contract layer would parallel-track the spreadsheet computation without adding correctness gain. The render-side `RenderedAnalysis` already exposes NOI (via the metrics surface or stress scenarios depending on which NOI the underwriter targets per Decision 4) and now exposes concludedCapRate (per Decision 3). The downstream Concluded Value computation can happen server-side as a future addition if needed, but isn't load-bearing for I9 scoping. Recorded explicitly to prevent scope creep.

**Deferred decisions (3) — recorded for implementation-time pickup:**

The decisions deferred here are implementation-time choices that depend on the broader infrastructure state at implementation time:

- **Field naming finalization** — `concludedCapRate` is the working name (matches legacy mapping name + AdjustedAssumptions field convention); implementation-time may surface alternatives or naming-collision concerns. Per the v15 Decision 4 / §14.2 precedent (Phase 2 AppraisalOperatingStatement field name "may rename during Phase 2 implementation per §13.4 considerations").
- **SourceTier value addition** — `'ANALYST_JUDGMENT'` is the proposed SourceTier label for analyst-input-without-engine-derivation. Implementation-time decides between adding `'ANALYST_JUDGMENT'` to the existing SOURCE_TIERS list vs reusing an existing tier (`'MANUAL'` if it exists) vs treating analyst-edited fields as a sub-property orthogonal to SourceTier. Implementation-time recon against the SOURCE_TIERS list resolves this.
- **Handbook-context UI placement** — the (γ) target's handbook-context-panel surface is Frontend territory. Specific placement (inline near I9 cell vs side-panel vs hover-card vs modal) depends on the render layer's editable-cell affordance shape at implementation time. May require coordination with #40 narrative producer work-in-progress if both target the same surface area.

**Future contract design conversations capture under §14.4, §14.5, etc.** This forward-looking placeholder language continues from §14.2 / §14.1 conventions.
...
- `<V16-HASH>` (2026-06-09) — v16: §14.3 I9 Concluded Cap Rate decisions document (third contract-decisions section after §14.1 PCA and §14.2 Appraisal; **flat 6-decision structure** returning to §14.1's pattern since I9 is single-phase per Delta S; 3 deferred decisions); §11.4 item 3 v16 layered note (Delta T closure — I9 reclassified out of producer-shape per Delta S; original v6 framing preserved per §13.6/§13.8 layering discipline); legacy-reduction-plan.md two-touch refresh (Delta U closure — Phase 1 item 6 addition for #24/7.3 AdjustedAssumptions render-side projection the original list missed + Phase 4 layered note acknowledging constituent mechanisms have opened piece-by-piece since #20 / v12 #24). Four deltas captured inline in §14.3's opening paragraphs (not new §13.x entries; consistent with v15's Delta P/Q-as-§13.5/§13.8-family treatment): R (legacy populator name collision; handled via Decision 5 leave-untouched), S (P-III-9 LLM_CONTEXT reclassification; load-bearing for the entire reframing), T (§11.4 item 3 framing stale; closed by v16 layered note), U (legacy-reduction-plan.md two-doc-surface drift across Phase 1 + Phase 4). **Drift-sweep discipline empirically validated** — yesterday's v15 closure proposed drift-sweep-before-composition as forward-discipline candidate; today's v16 Step 1 sweep (5-minute read across 6 doc surfaces) yielded one NEW finding (broader Delta U: Phase 1 missing #24/7.3) that v15's narrow framing missed; first documented empirical-confirmation of the discipline's value. The discipline doesn't earn a new §13.x entry; §14.3's opening paragraph records it as substantive contribution. No §10 entry per the v11 precedent for docs-only design/scoping amendments. v16 ships zero production code; pure scoping per the v8 / v15 precedent (third docs-only design/scoping amendment in project history). v16 amendment commit is docs-only across two files (SPEC.md + legacy-reduction-plan.md per v11 light-integration discipline keeping SPEC and operational doc synchronized); no SHIP-HASH or other code-side placeholders to resolve.
