...
- AdjustedAssumptions (rentGrowthPct, expenseGrowthPct, capRate, terminalCapRate) — backend-editable today via POST /:id/revisions but not projected to `RenderedAnalysis`. Tracked at [#24](https://github.com/isaint-jean/cre-credit-committee/issues/24).
...
- [#24](https://github.com/isaint-jean/cre-credit-committee/issues/24) — Expose AdjustedAssumptions in RenderedAnalysis + UI edit affordance. Phase-2-adjacent. 4 backend-editable assumption paths (capRate, terminalCapRate, rentGrowthPct, expenseGrowthPct) not yet projected to the rendered surface; AdjustedAssumptions section needs adding to `RenderedAnalysis` contract + render-version bump.
