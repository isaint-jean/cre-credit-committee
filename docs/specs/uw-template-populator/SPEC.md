...
- [#49](https://github.com/isaint-jean/cre-credit-committee/issues/49) — *to be filed in v13 Phase 2 close-out.* RecordGraphStore cast cleanup — architectural ticket split from [#45](https://github.com/isaint-jean/cre-credit-committee/issues/45) per the v13 recon reclassification (same split-pattern as #48 split from #45). 3 cast sites at `test-handbook-evaluation-route.ts:76` + `test-build-and-ingest-route.ts:210` + `:797` construct partial method-stub objects and cast to the full `RecordGraphStore` class. `RecordGraphStore` is exported as a class with ~50+ methods (not an interface); cleaning these casts cleanly requires production-code refactor — either interface segregation (extract `RecordGraphStoreReader` / `RecordGraphStoreWriter` / `HandbookEvaluationReader` etc. from the class) or `Pick<RecordGraphStore, ...>` narrowing at the route call sites. Per the recon recommendation, this is architectural work outside the "fixture cleanup" framing of #45; deferring to a dedicated #49 ticket preserves per-commit architectural clarity. Issue filing pending in v13 Phase 2 close-out alongside the #45 / #48 / #47 `gh issue close` calls.
