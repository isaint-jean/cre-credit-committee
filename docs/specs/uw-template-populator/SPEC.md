...
| E35-M35 (9 cells) | 10 Yr Pro Forma | Other Capital Expenditures per year (0 × 9) | Yes (Phase 2 via assembler projection) | `buildCapexScheduleInflated` (pure passthrough) + assembler-layer `bag['capex_projection']` activation | Pattern 3 (`capexScheduleInflated` → undefined when PCA absent) | No (populator gated on #41) | P-III-3 | **Updated (v9):** new in `f94d9f2` per §14.1 Decision 1. `AdjustedCapitalReserves` now carries `capexScheduleInflated` / `capexScheduleUninflated` as sibling arrays alongside the scalar `monthlyCapex`; assembler projects `capexScheduleInflated` into `bag['capex_projection']` (P-IV-RET-6's first array operand — see §10.9). KNOWN LIMITATION per JSDoc on `PCAExtraction.capexScheduleInflated`: hybrid two-call AI extraction achieves ~50-60% per-year alignment accuracy ceiling; sum-of-amounts is exact, per-year index is approximate. Follow-up #TBD tracks the improvement. |
...
Plus the implementation-time storage-table-schema-gap finding (Step 4 surfaced that `extraction_input_cache` doesn't have a `pca_hash` column even though the cache-key shape now includes `pca: ContentHash | null`; deferred to follow-up issue #TBD per the "scope-honesty" discipline §13.4 captures).
...
Until the fixture cleanup ships, prefer `!= null` at assembler-layer reads against `AdjustedInputs.*` fields. The follow-up issue #TBD tracks the fixture-discipline cleanup; when it ships, the runtime checks can be re-tightened to `!== null` since the runtime values will actually match the contract.
...
- `apps/api/fixtures/sunroad-centrum-pca.pdf` — Sunroad-Centrum PCA (committed `431102d`, Partner Engineering ASTM E2018-15 report). Anchor for §14.1 Decisions 1, 3, 5 and the `f94d9f2` end-to-end verification per §10.6.

**Follow-up issues (`#TBD` placeholders pending Sub-step 9.3 filing):**
- `#TBD` — PCA capex-schedule year-alignment improvement. Per §10.6 KNOWN LIMITATION: hybrid two-call AI extraction achieves ~50-60% per-year-alignment accuracy ceiling; sum-of-amounts is exact. Sub-step 9.3 will file as a tracking issue for a future prompt-engineering or constrained-decoding iteration. The Step 2 prompt-engineering history (3 iterations, with the "READ THE TOTALS ROW" iteration regressing from 6/12 to 4/12 correct) suggests the ceiling is real and may require an architectural change (constrained JSON Schema, finer-grained per-year prompts, or multi-pass alignment correction).
- `#TBD` — `extraction_input_cache` table `pca_hash` column migration. Per §13.5 Step 4 finding: the `ExtractionInputKeyArgs.slotHashes` shape was widened in `f94d9f2` to include `pca: ContentHash | null` for cache-distinguishing correctness, but the storage-table schema does not yet have a `pca_hash` column. Production impact today: cache lookups silently work because the `slotHashes` JSON blob is stored as a single column. Sub-step 9.3 files an explicit table-migration follow-up to bring the storage schema into line with the cache-key shape.
- `#TBD` — Test fixture cast-discipline cleanup. Per §13.6: replace `as unknown as HydratedRecordGraph` (and sibling-shape) casts with proper full-contract literals so runtime values match the contract. Until this ships, assembler-layer reads use loose `!= null` rather than strict `!== null`. Sub-step 9.3 files as a refactoring follow-up for the fixture sweep.
- `#TBD` — Missing npm scripts surfaced during PCA ticket. Specifics TBD pending Sub-step 9.3 audit; the package.json sweep during PCA implementation noted that some judgment-engine-adjacent scripts are not registered in the root `package.json`'s `scripts` object even though the underlying check files exist. Low-priority follow-up; not blocking any current work.

These placeholders are intentionally non-clickable in v9 prose; Sub-step 9.3 of the PCA ticket post-ship work files the four issues and updates the `#TBD` references in a follow-up `docs:` commit.
