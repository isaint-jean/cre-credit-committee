...
| E35-M35 (9 cells) | 10 Yr Pro Forma | Other Capital Expenditures per year (0 × 9) | Yes (Phase 2 via assembler projection) | `buildCapexScheduleInflated` (pure passthrough) + assembler-layer `bag['capex_projection']` activation | Pattern 3 (`capexScheduleInflated` → undefined when PCA absent) | No (populator gated on #41) | P-III-3 | **Updated (v9):** new in `f94d9f2` per §14.1 Decision 1. `AdjustedCapitalReserves` now carries `capexScheduleInflated` / `capexScheduleUninflated` as sibling arrays alongside the scalar `monthlyCapex`; assembler projects `capexScheduleInflated` into `bag['capex_projection']` (P-IV-RET-6's first array operand — see §10.9). KNOWN LIMITATION per JSDoc on `PCAExtraction.capexScheduleInflated`: hybrid two-call AI extraction achieves ~50-60% per-year alignment accuracy ceiling; sum-of-amounts is exact, per-year index is approximate. Follow-up [#44](https://github.com/isaint-jean/cre-credit-committee/issues/44) tracks the improvement. |
...
- [#44](https://github.com/isaint-jean/cre-credit-committee/issues/44) — PCA capex-schedule year-alignment improvement. Per §10.6 KNOWN LIMITATION: hybrid two-call AI extraction achieves ~50-60% per-year-alignment accuracy ceiling; sum-of-amounts is exact. Issue body documents the Step 2 prompt-engineering history (3 iterations, with the "READ THE TOTALS ROW" iteration regressing from 6/12 to 4/12 correct) and four proposed approaches (deterministic PDF table parsing eval, constrained decoding, multi-pass alignment correction, hybrid deterministic-skeleton + AI-fill-in).
- [#46](https://github.com/isaint-jean/cre-credit-committee/issues/46) — `extraction_input_cache` table `pca_hash` column migration. Per §13.5 Step 4 finding: the `ExtractionInputKeyArgs.slotHashes` shape was widened in `f94d9f2` to include `pca: ContentHash | null` for cache-distinguishing correctness, but the storage-table schema does not yet have a `pca_hash` column. Production impact today is debug-info-only (cache lookups already correctly distinguish PCA-vs-non-PCA via the composite `cache_key` hash); the missing column affects only admin / observability surface. Issue body lays out the migration shape via the existing `migrateAddColumns()` idempotent shim.
