...
const sectionII_principles: Principle[] = [
...

  {
    ...
    sourceCitation:
      'Handbook gate authored 2026-05-31; addresses Sunroad-scorecard finding that LLM principles asserted office stereotypes (phantom rollover, $0-reserves CRITICAL, missed footnote terminations) without reading the underlying deal records.',
    trigger: { kind: 'always' },
    executionModes: ['LLM_CONTEXT'],
    injectionPoints: [
      'committee_recommendation',
...
export const handbook: Handbook = {
  version: '2026.5-period-fix',
  effectiveDate: '2026-05-31',
  description:
    'Eightfold CRE Credit Handbook — structured form derived from atomization Sessions 1-4 (commits d183cb6, e2c84d0, 47da28e, plus the cross-reference cleanup pass). 87 atomic principles + 1 cluster narrative + 7 review steps capturing the full handbook. Source-of-truth typed TS literal; handbook.json is generated from this file. 2026.3-gate adds P-II-9 (read deal data before asset-class priors) as a meta-principle in the core_philosophy cluster, paired with the per-principle computedFacts surface introduced in HANDBOOK_ENGINE_VERSION 1.3.0. 2026.4-noi-recon adds P-III-15 (NOI reconciliation: underwriting uplift earns its way against trailing actuals) in the universal_framework cluster, paired with the noiReconciliation computedFacts entry introduced in HANDBOOK_ENGINE_VERSION 1.5.0. 2026.5-period-fix is a no-handbook-change cleanup release paired with HANDBOOK_ENGINE_VERSION 1.6.0: class-(a) downstream fields (reserves, taxes, reimbursements, vacancyLoss, totalOpEx) now cascade through the GS U/W column instead of the In-Place column, correcting silently-zero reserves on every deal whose CF carries both columns. Handbook principles unchanged; the source they read from is now correct.',
  clusters: [
    coreCluster,
    universalCluster,
    singleTenantCluster,
