...
function buildPrompt(text: string): string {
  return [
    ...
    '- buildingClass: scan prose for phrases like "Class A", "Class B/C",',
    '  "LEED certified class A office building", "an 11-story class A office',
    '  building", "class A-plus market appeal". Extract just the class letter',
    '  and optional grade modifier ("A", "B", "C", "A-", "B+", "C-"). Variants:',
    '  "Class A office" -> "A"; "Class B/C" or "Class B-C" -> "B" (pick the',
    '  higher of the two — defensive: a B-or-C asset is materially worse than',
    '  pure A so over-reporting class is the dangerous direction; never report',
