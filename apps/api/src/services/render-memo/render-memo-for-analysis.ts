...
export function renderMemoForAnalysis(
  ...
  if (snapshot !== null) {
    ...
    const html = buildCommitteeMemo({
      ...
      // Phase 2 — servicer narrative notes (site visit, broker feedback) as display-
      // only DD red flags; resolved via the analysis→loan join. Empty → memo byte-identical.
      servicerNotes: servicerNotesForAnalysis(analysis.graphRevisionId),
      renderSource,
