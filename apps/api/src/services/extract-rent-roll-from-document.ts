...
function buildPrompt(text: string, propertyHint: string | null): string {
  return [
    ...
    '      "isResidential": <true for a residential dwelling unit; false for an ancillary line (parking spot, storage locker, employee/leasing-office unit, common amenity). REQUIRED on every unit line.>',
    '      "status": "OCCUPIED" | "VACANT" | "PRELEASED" | "HOLDOVER" | "UNKNOWN",',
    '      "squareFeet": <number or null>,',
    '      "bedrooms": <number or null>,',
    '      "bathrooms": <number or null>,',
    '      "unitType": "<e.g. 1BR/1BA, Studio, Townhome, Parking, Storage — or null>",',
    '      "leaseStart": "<ISO date or null>",',
    ...
    '- ★ ISRESIDENTIAL tagging — distinguish RESIDENTIAL UNITS from ANCILLARY LINES:',
    '    isResidential=true:  dwelling unit (Studio, 1BR, 2BR, 3BR, townhome, single-family rental, etc.)',
    '    isResidential=false: parking spot (P-NN, lot numbers), storage locker (S-NN, locker numbers),',
    '                         employee unit / leasing office / model unit, common amenity space.',
    '  KEEP both kinds — do NOT drop ancillary lines (audit fidelity). The isResidential flag lets',
    '  downstream filter correctly. Setting an ancillary line as isResidential=true would DOUBLE COUNT',
    '  its income against the operating statement\'s "other income" line (parking + storage rents are',
    '  reported there separately).',
    '- Do NOT emit summary / total / subtotal rows (e.g. "TOTAL (24 units) ...") as units — those are',
    '  aggregations, not unit rows.',
