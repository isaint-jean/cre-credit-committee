...
export interface ResolveLoanForRootStores {
  readonly sqliteStore: Pick<SqliteStore, 'getRootRefAndName'>;
...
/**
 * Resolve a graph ROOT (`lineage_root_id`, 64-hex) to the single pool loan it
 * belongs to. Refuses (never guesses) when the resolution is not exactly one.
 */
