 ...
 *
 * Writes to data/phase6-addendum.db (SEPARATE from production
...
const DB_PATH = path.join(REPO, 'apps/api/data/phase6-addendum.db');
...
(async () => {
  console.log('============================================================');
  console.log('PHASE 6 — Sunroad addendum rerun (Rules A + B encoded)');
  console.log('============================================================');
  ...
  const composed = await buildExtractionResult({
    ...
    analysisAsOfDate: AS_OF,
    dealRef: 'SUNROAD-PHASE6-ADDENDUM',
